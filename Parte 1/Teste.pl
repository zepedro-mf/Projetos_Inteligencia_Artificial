% =========================================================================
% Projeto: Tensão Arterial (P1, P2, P3 integrados)
% Ficheiro: projeto_ta.pl
% Uso: consult('projeto_ta.pl').  Depois: ?- demo.
% =========================================================================
...
% diagnostico_risco: exemplo simples (usa hipertensao_registada abaixo)
hipertensao_registada(ID_P) :-
    consulta(_, _, ID_P, _, Diast, Sist, _),
    ( number(Diast), number(Sist) -> (Diast >= 90 ; Sist >= 140)
    ; % se tiver imprecisão usar análise intervalar para decidir possibilidade/certeza
...
+paciente(ID, Nome, Data, Sexo, Morada) :: (
    ( findall( (N,D,M), paciente(ID,N,D,Sexo,M), L),
      forall(member((N0,D0,M0), L), \+ (N0 == interdito ; D0 == interdito ; M0 == interdito))
    ).
...
% Invariante para consulta: paciente deve existir (ou não haver negação forte)
+consulta(_,_,P,_,_,_,_) :: (
    ( paciente(P,_,_,_,_) ; \+ neg(paciente(P,_,_,_,_)) )
).

% Invariante de coerência para consulta (quando aplicável)
+consulta(ID,Data,P,Idade,Diast,Sist,Puls) :: (
...
remover_consulta(ID) :-
    consulta(ID, Data, PID, Idade, Diast, Sist, Puls),
    involucao(consulta(ID, Data, PID, Idade, Diast, Sist, Puls)).
...
listar_consultas :-
    ...
    forall(consulta(ID, Data, PID, Idade, Diast, Sist, Puls),
           format('~w | Paciente: ~w | Data: ~w | ~w/~w | Puls: ~w~n', [ID, PID, Data, Sist, Diast, Puls])).

% relatório simples de um paciente (histórico + classificação por consulta)
relatorio_paciente(ID_P) :-
    paciente(ID_P, Nome, Data, Sexo, Morada),
    ...
    ; forall(member((DC,Id,Di,Si,Cl), L),
             format('~w | Idade: ~w | Tensão: ~w/~w | Class: ~w~n', [DC, Id, Si, Di, Cl]))
    ).
